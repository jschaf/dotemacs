...
;;  ada-support provides emacs 22 functions not defined in earlier versions of emacs
(if (not (ada-check-emacs-version 22 0))
    (progn
      (require 'ada-support)

      ;; The ff-special-construct for 'with' in find-file.el is wrong, so we delete it
      (delete
       '("^with[ \t]+\\([a-zA-Z0-9_\\.]+\\)" .
	 (lambda ()
...
(defcustom ada-stmt-end-indent 0
  ;; FIXME: how is this different from ada-broken-indent?
  ;; and the example is not "the end of a statement"!
  ...
  (cond
   ...
   ((and (not (bolp))
	...
	(setq source (point-marker)))
	  ...
	  (compilation-goto-locus error-pos source nil) ; Emacs 22
	(compilation-goto-locus (cons source error-pos))) ; Emacs 21
