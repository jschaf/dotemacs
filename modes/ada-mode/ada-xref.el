...
(defun ada-quote-cmd (cmd)
  ...
  ;; FIXME: use shell-quote-args instead
...
'default.adp'."
  ...
  (let (selected)
    ...
    ;; Emacs 21 doesn't have derived-mode-p
    (if (not (and (if (fboundp 'derived-mode-p)
		      (derived-mode-p 'ada-mode)
		    (string= mode-name "Ada")); derived-mode-p not in Emacs 21
		  buffer-file-name))
...
existing buffer `*gnatfind*', if there is one."
  ...
  ;;  processed (gnatfind \"+\":...).
  ;;  FIXME: use shell-quote-arg instead
  (let* ((quote-entity
	  (if (= (aref entity 0) ?\")
	      (if is-windows
	...
	(let ((compilation-error "reference")) ;; _not_ capitalized, despite checkdoc
	  (compilation-start command 'compilation-mode (lambda (mode) ada-gnatfind-buffer-name)))
      ;; emacs 21
      (compile-internal command "No more references" "compilation"
