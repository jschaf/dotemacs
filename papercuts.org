* add rebase to evil emacs state
* update highlight-symbol when theme changes
* ignore keywords, number and other unintersting symbols in highlight-symbol 
* exclude most block based languages from highlight paren
* use jk to exit y-or-no-p
