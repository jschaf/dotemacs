...
(defface cycle-buffer-selected
  '((t :foreground "skyblue3"))
  "The face used to highlight the current buffer in the selection")
...
(defun cycle-buffer-show (permissive)
  ...
  (let (wid list str s mid current next p n)
    ...
    (while (and list (not (and p n (eq (car p) (car n))))
      ...
      (setq s (if current
                  (propertize (format
                               (car cycle-buffer-show-format)
                               (cycle-buffer-shorten-name (buffer-name s)))
                              'face 'cycle-buffer-selected)
                (format (cdr cycle-buffer-show-format)
                        (cycle-buffer-shorten-name (buffer-name s)))))
